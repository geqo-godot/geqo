...
void QueryTest2D::_bind_methods() {
	...
	ADD_PROPERTY(PropertyInfo(Variant::INT, "filter_type", PROPERTY_HINT_ENUM, "Range,Min,Max"), "set_filter_type", "get_filter_type");
...
void QueryTest2D::_validate_property(PropertyInfo &property) const {
	...
	if (numeric_vars.has(property.name))
		if (get_test_type() == TEST_TYPE_BOOLEAN || get_test_purpose() == SCORE_ONLY)
