		...
		case NOTIFICATION_READY: {
			...
			if (!Engine::get_singleton()->is_editor_hint()) {
				...
				for (Variant child : get_children()) {
					ContextQuerier2D *context_querier = cast_to<ContextQuerier2D>(child);
					if (context_querier) {
				...
				if (!has_querier_context) {
					set_querier_context(memnew(ContextQuerier2D));
...
void EnvironmentQuery2D::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("set_debug_lifetime_sec", "sec"), &EnvironmentQuery2D::set_debug_lifetime_sec);
	ClassDB::bind_method(D_METHOD("get_debug_lifetime_sec"), &EnvironmentQuery2D::get_debug_lifetime_sec);
