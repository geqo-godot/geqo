...
void GEQODebugSpheres2D::draw_items(vector<QueryItem2D> &query_items_list, double time_to_destroy) {
	remove_labels();

	debug_draw->set_query_items(query_items_list);
	debug_draw->set_destroy_time(time_to_destroy);

	for (QueryItem2D &query_item : query_items_list) {
...
void GEQODebugDraw2D::set_query_items(vector<QueryItem2D> &items) {
...
void GEQODebugDraw2D::_draw() {
	...
	for (QueryItem2D &query_item : saved_query_items) {
		Color color;
		if (query_item.get_is_filtered()) {
			color = Color(0, 0, 1);
		} else if (query_item.get_has_score()) {
