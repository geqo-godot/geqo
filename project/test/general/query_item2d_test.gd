...
func test_create_initializes_all_fields() -> void:
	...
	var pos: Vector2 = Vector2(1, 2)
	var item: QueryItem2D = QueryItem2D.create(pos, node)
...
func test_create_with_null_collider() -> void:
	...
	var item: QueryItem2D = QueryItem2D.create(Vector2.ONE, null)
...
func test_property_roundtrip() -> void:
	...
	var item: QueryItem2D = QueryItem2D.create(Vector2.ZERO, null)
	var node: Node2D = auto_free(Node2D.new())
	item.collided_with = node
	item.projection_position = Vector2.ONE
	item.score = 42.0
	item.is_filtered = true
	assert_object(item.collided_with).is_same(node)
	assert_vector(item.projection_position).is_equal(Vector2.ONE)
...
func test_add_score_numeric_increases_score_in_range() -> void:
	...
	var item := QueryItem2D.create(Vector2.ZERO, null)
